...
func NewDefault() *Registry {
	r := NewRegistry()
	for _, group := range [][]Descriptor{builtinLight, builtinMem, builtinLoad, builtinPressure, builtinFD, builtinSocket, builtinGPU, builtinEvent, builtinPerf, builtinPower} {
		for _, d := range group {
			r.MustRegister(d)
		}
	}
